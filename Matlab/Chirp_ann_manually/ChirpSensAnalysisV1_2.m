...
% ann_output = predict(netChirp,dlarray{InputData});
...
load chirp_mlp_dl_model.mat; % Model Build manually in dL_manually.m script
...
X = dataSet_chirp_30params_test25db_numberedLabel(:,1:30); % only params, not labels
Xstandardized = (X - mu_parameter) ./ sigma_parameter;

% Transpose dataset to fit with Ann model input
TestMat_Chirp_25dB = Xstandardized(:,1:30)'; 
% *************************************************************************
...
% Extract every Data labeled Batch to calculate u, sigma and std
   ...
   dimBatch    = size(TestMat_Chirp_25dB,2);  % 5000 Full size
...
for jClass = 1: NUM_CLASS
    muMat_inputESSC(:,jClass)    = mean(TestMat_Chirp_25dB(: , ( (((subdimBatch * jClass)+1)-subdimBatch) : (subdimBatch * jClass) )),2);  % Mean "mu"
    sigmaMat_inputESSC(:,jClass) = std(TestMat_Chirp_25dB (: , ( (((subdimBatch * jClass)+1)-subdimBatch) : (subdimBatch * jClass) )),1,2); % std desviation "sigma"
end
% eliminar luego

% muMat_inputESSC(:,1) = TestMat_Chirp_25dB(:,1);
% muMat_inputESSC(:,2) = TestMat_Chirp_25dB(:,1001);
% muMat_inputESSC(:,3) = TestMat_Chirp_25dB(:,2001);
% muMat_inputESSC(:,4) = TestMat_Chirp_25dB(:,3001);
% muMat_inputESSC(:,5) = TestMat_Chirp_25dB(:,4001);
