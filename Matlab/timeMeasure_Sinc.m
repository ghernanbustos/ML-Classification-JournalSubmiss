...
load sinc_exportedTrainedModels_BIS.mat; % Loads Trainned Models
% Loads One Raw Sinc DataWaveform (amp, time) with Gaussian deformation type.
load sinc_gauss1_25dB_testElement.mat; 

% Manual extraction to be use by f_net
esscParams = essc_computingTime(sinc_gauss1_time, sinc_gauss1_amp); 
% -------------------------------------------------------------------------
nTimer = 100;
...
% Function Handle ---------------------------------------------------------
f_essc = @() essc_computingTime(sinc_gauss1_time, sinc_gauss1_amp); 
f_net = @() net_computingTime(Sinc_MLP_30ESSC_trainedModel.predictFcn, esscParams); % handle to function
...
f_essc = @() essc_computingTime(sinc_gauss1_time, sinc_gauss1_amp); 
f_net = @() net_computingTime(Sinc_SVM_30ESSC_trainedModel.predictFcn, esscParams); % handle to function
...
f_essc = @() essc_computingTime(sinc_gauss1_time, sinc_gauss1_amp); 
f_net = @() net_computingTime(Sinc_NB_30ESSC_trainedModel.predictFcn, esscParams); % handle to function
...
f_essc = @() essc_computingTime(sinc_gauss1_time, sinc_gauss1_amp); 
f_net = @() net_computingTime(Sinc_KNN_30ESSC_trainedModel.predictFcn, esscParams); % handle to function
...
save('Sinc_timeModelMAt_essc','timeModelMAt_essc');
save('Sinc_timeModelMAt_net','timeModelMAt_net');
save('Sinc_timeModelMAt_endToend','timeModelMAt_endToend');
