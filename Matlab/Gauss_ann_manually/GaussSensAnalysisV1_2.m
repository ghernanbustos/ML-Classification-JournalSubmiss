...
% ann_output = predict(netGauss,dlarray{InputData});
...
load gauss_workspace.mat; % DataSets
load gauss_mlp_dl_model.mat; % Model Build manually in dL_manually.m script
...
X = dataSet_gauss_30params_test25db_numberedLabel(:,1:30); % only params, not labels
Xstandardized = (X - mu_parameter) ./ sigma_parameter;
% -------------------------------------------------------------------------

% Transpose dataset to fit with Ann model input
TestMat_Gauss_25dB = Xstandardized(:,1:30)'; 
% *************************************************************************
...
% Extract every Data labeled Batch to calculate u, sigma and std
   ...
   dimBatch    = size(TestMat_Gauss_25dB,2);  % 5000 Full size
...
for jClass = 1: NUM_CLASS
    muMat_inputESSC(:,jClass)    = mean(TestMat_Gauss_25dB(: , ( (((subdimBatch * jClass)+1)-subdimBatch) : (subdimBatch * jClass) )),2);  % Mean "mu"
    sigmaMat_inputESSC(:,jClass) = std(TestMat_Gauss_25dB (: , ( (((subdimBatch * jClass)+1)-subdimBatch) : (subdimBatch * jClass) )),1,2); % std desviation "sigma"
end
% eliminar luego

% muMat_inputESSC(:,1) = TestMat_Gauss_25dB(:,1);
% muMat_inputESSC(:,2) = TestMat_Gauss_25dB(:,1001);
% muMat_inputESSC(:,3) = TestMat_Gauss_25dB(:,2001);
% muMat_inputESSC(:,4) = TestMat_Gauss_25dB(:,3001);
% muMat_inputESSC(:,5) = TestMat_Gauss_25dB(:,4001);
...
%          Invert the normalization.
        ...
        % -----------------------------------------------------------------
         ...
         if ( annOutput_piMat(jClass,1) < 0.9 )% positive Sigma
...
%          Invert the normalization.
        ...
        % -----------------------------------------------------------------
         annOutput_piMat(1:5,1) = predictedRawSoftmax;
         if ( annOutput_piMat(jClass,1) < 0.9 )% negative Sigma
            deltaSens = deltaSigmaArray(i_deltaSigma);% Save Sigma value
            break
         end 
      end 
